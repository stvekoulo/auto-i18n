...
export function renderCheckReport(projectRoot: string, report: CheckReport): void {
  logger.info(
    `${report.filesScanned} fichier${report.filesScanned > 1 ? 's' : ''} scanné${report.filesScanned > 1 ? 's' : ''}`,
  );
  ...
  if (report.uncatalogued.length > 0) {
    logger.warn(
      `${report.uncatalogued.length} string${report.uncatalogued.length > 1 ? 's' : ''} non encore cataloguée${report.uncatalogued.length > 1 ? 's' : ''} — lancez "sync"`,
    );
  ...
  if (report.sourceKeyCount > 0) {
    logger.info(
      `${report.sourceKeyCount} clé${report.sourceKeyCount > 1 ? 's' : ''} dans le catalogue source`,
    );
  }
  ...
  for (const [locale, missing] of Object.entries(report.missingByLocale)) {
    if (missing.length === 0) {
      ...
      logger.warn(
        `${locale} — ${missing.length} clé${missing.length > 1 ? 's' : ''} manquante${missing.length > 1 ? 's' : ''}`,
      );
    }
  }
